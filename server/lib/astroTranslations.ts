...
const PLANET_TRANSLATIONS: Record<string, Record<PlanetName, string>> = {
  en: {
    'Sun': 'Sun',
    'Moon': 'Moon',
    'Mercury': 'Mercury',
...
const SIGN_TRANSLATIONS: Record<string, Record<SignName, string>> = {
  en: {
    'Aries': 'Aries',
    'Taurus': 'Taurus',
    'Gemini': 'Gemini',
...
export function translatePlanet(planetName: string, locale: string): string {
  const normalizedName = planetName.trim() as PlanetName;
  return PLANET_TRANSLATIONS[locale]?.[normalizedName] ?? planetName;
}

export function translateSign(signName: string, locale: string): string {
